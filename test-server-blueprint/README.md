# Test server blueprint — stub SSO hub (local)

Applicazione Flask **minimale** che imita il **portale / hub SSO** in laboratorio (`/stub-portal` emette JWT verso il client UseIt). In produzione il vero hub è in rete; il client punta `PORTAL_URL` a quell’host.

Il progetto resta anche **template** per integrare SSO in altre app Flask: contiene l’ossatura — autenticazione,
