  ...
  describe('Drag and Drop Elements (Requirements 20.1, 20.2, 20.3)', () => {
    ...
    it('should validate complete drag-and-drop ARIA compliance', () => {
      ...
      dragHandlers.forEach(eventType => {
        const hasHandler = // @ts-expect-error: getEventListeners is a non-standard Chrome DevTools API not in TypeScript types
          (compliantDragDropElement as unknown as Record<string, unknown>)['getEventListeners']?.(eventType)?.length > 0 ||
