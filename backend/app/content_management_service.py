...
class ContentManagementService:
    ...
    async def index_content(
        ...
        """
        Chunk text, generate embeddings, and index into the folder's Qdrant collection.
        ...
        # Generate embeddings
        vectors = embed_text_batch(chunks)
        if not vectors:
            return 0
        ...
        for chunk_idx, (chunk_text_content, vector) in enumerate(zip(chunks, vectors)):
        ...
        logger.info(
            f"Indexed {len(points)} chunks for file {file_record.id} "
