...
class EnhancedSearchService:
    ...
    def _point_to_result(hit: Any, source: str = "qdrant") -> SearchResult:
        ...
        # Prefer original_filename as title so results show the PDF name, not a UUID
        title = (
    ...
    def _deduplicate_by_file(results: List[SearchResult]) -> List[SearchResult]:
        """
        Keep only the best-scoring chunk per file_id.

        This prevents returning 10 chunks from the same PDF — instead the
        user sees the most relevant excerpt from each distinct document.
        """
        best_per_file: Dict[str, SearchResult] = {}
        no_file_id: List[SearchResult] = []
        ...
        for r in results:
            ...
            if not file_id:
                no_file_id.append(r)
                continue
            if file_id not in best_per_file or r.score > best_per_file[file_id].score:
                best_per_file[file_id] = r

        return list(best_per_file.values()) + no_file_id

    @staticmethod
    def _deduplicate_by_filename(results: List[SearchResult]) -> List[SearchResult]:
        """
        When searching across all folders (no folder filter), keep only the
        best-scoring result per original_filename.

        This prevents showing the same PDF twice just because it was uploaded
        to multiple folders.
        """
        best_per_name: Dict[str, SearchResult] = {}
        no_name: List[SearchResult] = []

        for r in results:
            name = str(r.metadata.get("original_filename") or r.title or "").lower().strip()
            if not name:
                no_name.append(r)
                continue
            if name not in best_per_name or r.score > best_per_name[name].score:
                best_per_name[name] = r

        return list(best_per_name.values()) + no_name
    ...
    async def semantic_search(
        ...
        for collection in collections:
            try:
                hits = self._client.search(
                    ...
                    limit=(limit + offset) * 5,  # fetch more to allow per-file dedup
                    with_payload=True,
                )
                for hit in hits:
        ...
        # Deduplicate by file_id first, then by chunk id, then boost + sort
        deduped = self._deduplicate_by_file(self._deduplicate(all_results))
        # If no folder filter, also deduplicate same filename across folders
        if not folder_filter:
            deduped = self._deduplicate_by_filename(deduped)
        results = self._apply_ranking_boosts(deduped)
    ...
    async def keyword_search(
        ...
        for collection in collections:
            try:
                points, _ = self._client.scroll(
                    ...
                    limit=limit * 5,  # fetch more to allow per-file dedup
                    with_payload=True,
                    with_vectors=False,
                )
                for point in points:
                    ...
                    # Use original_filename as title (same as semantic search)
                    title = (
                        payload.get("original_filename")
                        or payload.get("title")
                        or payload.get("filename")
        ...
        results = self._apply_ranking_boosts(self._deduplicate_by_file(self._deduplicate(all_results)))
        if not folder_filter:
            results = self._deduplicate_by_filename(results)
    ...
    async def hybrid_search(
        ...
        results = self._apply_ranking_boosts(self._deduplicate_by_file(combined))
        if not folder_filter:
            results = self._deduplicate_by_filename(results)
        return results[:limit]
